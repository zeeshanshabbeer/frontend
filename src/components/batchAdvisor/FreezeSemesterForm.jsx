...
const FreezeSemesterForm = () => {
  const sendEmail = async (e) => {
    ...
    if (data.status === "success") {
      emailjs.sendForm("FreezeSemester_Service", "FreezeSemester_Email", e.target, "user_8Oqr3FKbWRDqndN2Yr0x7").then(
        (result) => {
  ...
  const FreezeForm = async () => {
    const res = await fetch(`https://backend-three-nu.vercel.app/FreezeSemester/Freeze_Form/${registrationId}`, {
      method: "GET",
      headers: {
  ...
  const submit = async (e) => {
    e.preventDefault();
    const res = await fetch("https://backend-three-nu.vercel.app/FreezeSemester/FreezeSemester_reject", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        authorization: localStorage.getItem("BA_token"),
      ...
      <div className='FreezeSemesterFormContainer'>
        <form action='' onSubmit={sendEmail}>
          <div className='credentials'>
            ...
            <input type='text' name='name' className='formDataName' value={freezeForm.name} />
          </div>
          <div className='credentials'>
            <h3 className='formLabel'>Reg No:</h3>
            <input type='text' name='regno' className='formData' value={freezeForm.registrationId} />
          </div>
          <div className='credentials'>
            <h3 className='formLabel'>Section:</h3>
            <input type='text' name='section' className='formData' value={freezeForm.section} />
          </div>
          <div className='credentials'>
            <h3 className='formLabel'>Email:</h3>
            <input type='text' name='email' className='formDataEmail' value={freezeForm.email} />
          </div>
          <div className='credentials'>
            <h3 className='formLabel'>CGPA:</h3>
            <input type='text' name='cgpa' className='formDataCgpa' value={freezeForm.CGPA} />
          </div>
          <div className='credentials'>
            <h3 className='formLabel'>Contact:</h3>
            <input type='text' name='contact' className='formDataContact' value={freezeForm.contactNo} />
          </div>
          <div className='credentials2'>
            <h3 className='formLabel'>Continuation Semester:</h3>
            <input type='text' name='continuationtime' className='FreezeTime' value={freezeForm.continuationTime} />
          </div>
          <h3 className='formreasonLabel'>Reason:</h3>
          <div className='credentials3'>
            <textarea cols={37} rows={10} type='text' name='reason' className='FreezeFormReason' value={freezeForm.reason} />
          </div>
          <button className='FreezeAcceptBtn'>Accept</button>
        </form>
        <div className='rejectbtndiv'>
          <button className='FreezeRejectBtn' onClick={() => setButtonPopup(true)}>
            Reject
          </button>
