...
const BatchAdvisorMailBox = () => {
  ...
  const Querybox = async () => {
    const res = await fetch("https://backend-three-nu.vercel.app/ChatBox/BA_ViewMessage", {
      method: "GET",
      headers: {
  ...
  const deleteSubmit = async (e) => {
    ...
    const res = await fetch(`https://backend-three-nu.vercel.app/ChatBox/BA_DeleteChat/${registrationId}/${subject}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
  ...
  return (
    <div className='BAprofilecontainer'>
            ...
            {querybox.map((query) => (
              <tr>
                <td className='highlightedRow'>
                  <Link to={"/BatchAdvisorQueryThread"} state={[query.registrationId, query.subject, query.chat[0].name]} className='highlightedRow'>
      ...
      <DeleteMessagePopup trigger={buttonPopup} setTrigger={setButtonPopup}>
        <form action=''>
          <h3 className='DeleteConfirmation'>Are you sure you want to delete this query?</h3>
