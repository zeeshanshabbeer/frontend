...
const StudentLogin = () => {
  ...
  const loginUser = async (e) => {
    ...
    const res = await fetch("https://backend-three-nu.vercel.app/Student/Studentlogin", {
      method: "POST",
      headers: {
    ...
    if (data.status === "success") {
      localStorage.setItem("S_token", `Bearer ${data.token}`);
      navigate("/Home");
    } else {
      Error(data.message);
    }
    ...
    <div className='maincontainer'>
      ...
      <div className='container2'>
        ...
        <form action='' method='POST'>
          ...
          <select className='batchinput' name='batch' id='' value={user.batch} onChange={handleInputs} placeholder=''>
          ...
          <input className='department' name='' type='text' placeholder='BCS' disabled />
          <input className='regNo' name='regNo' type='text' value={user.regNo} onChange={handleInputs} placeholder='' />
          <br /> <br />
          <input className='password' name='password' type='password' value={user.password} onChange={handleInputs} placeholder='Password' />
          <br />
