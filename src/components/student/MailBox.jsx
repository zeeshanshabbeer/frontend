...
const MailBox = () => {
  ...
  const Querybox = async () => {
    const res = await fetch("https://backend-three-nu.vercel.app/ChatBox/S_ViewMessage", {
      method: "GET",
      headers: {
  ...
  const deleteSubmit = async (e) => {
    ...
    const res = await fetch(`https://backend-three-nu.vercel.app/ChatBox/S_DeleteChat/${subject}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        authorization: localStorage.getItem("S_token"),
        ...
        {querybox.length === 0 ? (
          ...
          <table style={{ width: "100%", marginLeft: "0px" }}>
            ...
            {querybox.map((query) => (
              <tr>
                <td>
                  <Link to={"/QueryThread"} state={[query.subject, query.chat[0].name]} className='subjectRow'>
                    {query.subject}
                  </Link>
                </td>
